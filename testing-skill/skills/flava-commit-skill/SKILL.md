...
## What to commit (codebase only)

**Default:** Stage and commit only changes that belong to the **shared product codebase** — e.g. `apps/`, `packages/`, `shared/`, `scripts/`, team-owned repo config and CI under root when appropriate.

**Do not stage or commit** (treat as personal / local / AI tooling) **unless the user explicitly asks** for a separate commit that includes them:

| Category | Typical paths / patterns |
| -------- | ------------------------ |
| **AI agents & assistant layout** | `.claude/` (skills, errors, local agent config), `.cursor/` (rules, skills, hooks), `.codex/` and any path whose primary purpose is IDE/agent workflows |
| **Agent instruction docs** | Root or product `CLAUDE.md`, `.cursorrules`, or similar when they are assistant instructions rather than product documentation — **if unsure, ask once** |
| **Secrets & local env** | `.env`, `.env.*`, secrets, credentials, machine-specific local overrides |

When `git status` mixes codebase and agent files:

- **Stage by path** — e.g. `git add apps/product-lb/...` — instead of `git add -A`.
- **List what was excluded** after the commit so the user can keep personal changes locally.
...
6. **Stage if needed** — follow **[What to commit (codebase only)](#what-to-commit-codebase-only)**. Prefer explicit paths. Always skip `.env`, secrets, and agent/IDE-only trees unless the user explicitly requests otherwise.
